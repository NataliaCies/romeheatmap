...
class Settings(BaseSettings):
    ...
    allowed_origins: list[str] = [
        "https://vocal-marshmallow-7db69a.netlify.app",
        "http://localhost:3000",
    ]

    # ── Copernicus auth ───────────────────────────────────────────────────────
    # Option A (recommended): username + password → works for search AND download
    copernicus_user: str = ""
    copernicus_password: str = ""

    # Option B: OAuth client → works for search only (NOT zipper download)
    ...
    def rome_bbox(self) -> tuple[float, float, float, float]:
        return (self.rome_bbox_lon_min, self.rome_bbox_lat_min,
                self.rome_bbox_lon_max, self.rome_bbox_lat_max)
